"""
75.pyのパディング処理を活用し, ミニバッチでAvgEmbeddingClassifierを学習するスクリプト.
...
if __name__ == "__main__":
    ...
    train_minibatch(model, train_dataset, n_epochs=10, lr=1e-3, batch_size=32)
    torch.save(model.state_dict(), checkpoint_path)
    print(f"チェックポイントを {checkpoint_path} に保存しました.")

    acc = evaluate(model, dev_dataset, batch_size=32)
