...
def move_answer_d(row):
    original_answer = row['answer']

    if original_answer != "D":
        row["D"], row[original_answer] = row[original_answer], row["D"]
        row["answer"] = "D"

    return row


df_fixed = df.apply(move_answer_d, axis=1)

correct_count = 0
total = len(df)

for index, row in tqdm(df_fixed.iterrows(), total=total):
    retry_count = 0
    while True:
        try:
            prompt = f"""
            ...
            error_msg = str(e)
            if "429" in error_msg or "ResourceExhausted" in error_msg or "500" in error_msg or "INTERNAL" in error_msg:
                retry_count += 1
