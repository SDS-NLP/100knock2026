...
叔父《おじ》は仕方なしに私の空家《あきや》へはいる事を承諾してくれました。しかし市《し》の方にある住居《すまい》もそのままにしておいて、両方の間を往《い》ったり来たりする便宜を与えてもらわなければ困るといいました。私に固《もと》より異議のありようはずがありません。私はどんな条件でも東京へ出られれば好《い》いくらいに考えていたのです。
...
奥さんと私はできるだけの手際《てぎわ》と工夫を用いて、Ｋの室《へや》を掃除しました。彼の血潮の大部分は、幸い彼の蒲団《ふとん》に吸収されてしまったので、畳はそれほど汚れないで済みましたから、後始末はまだ楽でした。二人は彼の死骸《しがい》を私の室に入れて、不断の通り寝ている体《てい》に横にしました。私はそれから彼の実家へ電報を打ちに出たのです。
...
波瀾《はらん》も曲折もない単調な生活を続けて来た私の内面には、常にこうした苦しい戦争があったものと思って下さい。妻《さい》が見て歯痒《はがゆ》がる前に、私自身が何層倍《なんぞうばい》歯痒い思いを重ねて来たか知れないくらいです。私がこの牢屋《ろうや》の中《うち》に凝《じっ》としている事がどうしてもできなくなった時、またその牢屋をどうしても突き破る事ができなくなった時、必竟《ひっきょう》私にとって一番楽な努力で遂行《すいこう》できるものは自殺より外《ほか》にないと私は感ずるようになったのです。あなたはなぜといって眼を※《みは》るかも知れませんが、いつも私の心を握り締めに来るその不可思議な恐ろしい力は、私の活動をあらゆる方面で食い留めながら、死の道だけを自由に私のために開けておくのです。動かずにいればともかくも、少しでも動く以上は、その道を歩いて進まなければ私には進みようがなくなったのです。
...
if __name__=="__main__":
    ...
    sentences = parse_text(cleaned_text)
    documents = []
    for sentence in sentences:
        nouns = [morph['base'] for morph in sentence if morph['pos'] == '名詞']
        if nouns:
            documents.append(nouns)
    total_docs = len(documents)
    
    idf_scores = {}
    all_nouns = set(noun for doc in documents for noun in doc)
    for noun in all_nouns:
        docs_containing_word = sum(1 for doc in documents if noun in doc)
        idf_scores[noun] = math.log10(total_docs / docs_containing_word)
    max_tfidf_scores = {}
    for doc in documents:
        total_words = len(doc)
        tf_counts = Counter(doc)
        for noun, count in tf_counts.items():
            tf = count / total_words
            idf = idf_scores[noun]
            tfidf = tf * idf
            
            if noun not in max_tfidf_scores or tfidf > max_tfidf_scores[noun][2]:
                max_tfidf_scores[noun] = (tf, idf, tfidf)
    sorted_tfidf = sorted(max_tfidf_scores.items(), key=lambda x: x[1][2], reverse=True)
    
    for word, scores in sorted_tfidf[:20]:
        tf, idf, tfidf = scores
        print(f"{word:　<8} | {tf:.4f}   | {idf:.4f}   | {tfidf:.4f}")