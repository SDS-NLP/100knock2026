...
weights = logit_model.coef_[0]

top20_idx = np.argsort(weights)[-20:][::1]
bottom20_idx= np.argsort(weights)[:20]


print("=== Top 20 (Positive側) ===")
for i in top20_idx:
    print(f"{weights[i]:+.4f}  {feature_names[i]}")
