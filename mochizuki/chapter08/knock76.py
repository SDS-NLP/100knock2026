...
def accuracy(model, examples, batch_size=256, device="cpu"):
    loader = DataLoader(
...
def train(args):
    ...
    model = BoWClassifier(embeddings, freeze=True)
    loader = DataLoader(
        data["train"],
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        dev_acc = accuracy(model, data["dev"], args.batch_size)
        print(
            f"epoch {epoch:2d}  loss={total_loss / n:.4f}  "
            f"train_acc={correct / n:.4f}  dev_acc={dev_acc:.4f}"
        )

    torch.save(model.state_dict(), BASE_DIR / "model76.pt")
    print(f"saved: {BASE_DIR / 'model76.pt'}")
    print(f"dev accuracy: {accuracy(model, data['dev'], args.batch_size):.4f}")
